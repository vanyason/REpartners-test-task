...
//     Think of it as: "what's the best way to make change for amount t?"
//
...
func CalcPacks(items int, packs []int) (PacksCount, error) {
	...
	slices.SortFunc(cleaned, func(a, b int) int { return b - a })
	...
	dpSize := smallest * secondSmallest
	dpSize = max(dpSize, reduced[0]) // at least as large as the biggest reduced pack
	...
	for t := 1; t <= dpSize; t++ {
		...
		for _, p := range reduced {
			...
			candidate := prev.totalPacks + 1
			if !best.valid || candidate < best.totalPacks {
				best = dpEntry{totalPacks: candidate, lastPack: p, valid: true}
			}
		}
		dp[t] = best
	}
